...
func realtimeHTMLResponse() pluginapi.ManagementResponse {
	...
	body.WriteString("<script>")
	body.WriteString(`(function(){let masked=true,failed=false,openRow="";const content=document.getElementById("content"),rowsBadge=document.getElementById("row-count"),titleBadge=document.getElementById("row-count-title"),acctBadge=document.getElementById("account-count"),maskBtn=document.getElementById("mask"),failedBtn=document.getElementById("failed"),refreshBtn=document.getElementById("refresh");function esc(v){return String(v==null?"":v).replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[c]))}function fmtMS(v){v=Number(v||0);return v>=1000?(v/1000).toFixed(v>=10000?1:2)+" s":Math.round(v)+" ms"}function usage(u){u=u||{};return {i:Number(u.input_tokens||0),o:Number(u.output_tokens||0),t:Number(u.total_tokens||0)}}function time(v){if(!v)return "";const d=new Date(v);return d.toLocaleDateString()+"<br>"+d.toLocaleTimeString()}function detail(row,id){return '<tr class="details" id="d-'+esc(id)+'"><td colspan="10"><pre>'+esc(JSON.stringify(row.details||[],null,2))+'</pre></td></tr>'}function rowHTML(row,i){const u=usage(row.usage),id=String(row.id||("row-"+i)),status=row.status==="Success"?'<span class="ok">Success</span>':'<span class="badtext">Failed</span>',err=row.error?'<div class="sub badtext">'+esc(row.error)+'</div>':"";return '<tr class="row"><td>'+time(row.time)+'</td><td><div class="source">'+esc(row.source)+'</div><div class="sub">Provider: venice</div>'+err+'</td><td><span class="model">'+esc(row.model)+'</span></td><td><div class="effort">'+esc(row.effort||"medium")+'</div><div class="small">Tier: '+esc(row.tier||"default")+'</div></td><td>'+status+'</td><td><span class="metric">'+fmtMS(row.ttft_ms)+'</span></td><td><span class="metric">'+fmtMS(row.elapsed_ms)+'</span></td><td class="num">'+Number(row.tps||0).toFixed(0)+'</td><td><div class="usage">'+(u.t>=1000?(u.t/1000).toFixed(1)+"K":u.t)+'</div><div class="small">I '+u.i+' / O '+u.o+'</div></td><td><span class="toggle" data-id="'+esc(id)+'">Details</span></td></tr>'+(openRow===id?detail(row,id):"")}function render(data){const rowCount=(data.summary&&data.summary.rows)||0;rowsBadge.textContent=rowCount;titleBadge.textContent=rowCount;acctBadge.textContent=(data.summary&&data.summary.accounts)||0;if(!data.rows||!data.rows.length){content.className="empty";content.textContent="Waiting for Venice requests...";return}content.className="";content.innerHTML='<table class="table"><thead class="head"><tr><th>Time</th><th>Source</th><th>Model</th><th>Effort</th><th>Status</th><th>TTFT</th><th>Elapsed</th><th>TPS</th><th>Usage</th><th>Detail</th></tr></thead><tbody>'+data.rows.map(rowHTML).join("")+'</tbody></table>';content.querySelectorAll(".toggle").forEach(el=>el.onclick=()=>{openRow=openRow===el.dataset.id?"":el.dataset.id;load()})}async function load(){const url="realtime.json?rows=500&masked="+(masked?1:0)+"&failed="+(failed?1:0)+"&_="+Date.now();try{const res=await fetch(url,{cache:"no-store"});render(await res.json())}catch(e){content.className="empty";content.textContent=String(e&&e.message?e.message:e)}}maskBtn.onclick=()=>{masked=!masked;maskBtn.classList.toggle("off",!masked);maskBtn.textContent=masked?"Masked":"Unmasked";load()};failedBtn.onclick=()=>{failed=!failed;failedBtn.classList.toggle("off",!failed);load()};refreshBtn.onclick=load;load();setInterval(load,1000);})();`)

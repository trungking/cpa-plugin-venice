...
func realtimeHTMLResponse() pluginapi.ManagementResponse {
	...
	body.WriteString("<style>:root{color-scheme:dark}*{box-sizing:border-box}body{font-family:system-ui,-apple-system,Segoe UI,sans-serif;margin:0;padding:24px;color:#e7eaf1;background:#101723;font-size:14px}.page{max-width:1680px;margin:0 auto}.bar{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:18px}.title{display:flex;align-items:center;gap:10px}h1{font-size:18px;line-height:1.2;margin:0;font-weight:800}.count{background:#0f4777;color:#9bd0ff;border-radius:999px;padding:4px 9px;font-weight:800;font-size:12px}.actions{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.seg{display:flex;border-radius:9px;overflow:hidden;background:#1e2634;border:1px solid #293346}.seg a,.seg span{display:flex;align-items:center;gap:8px;padding:8px 13px;color:#9da7b8;font-weight:800;font-size:12px;text-decoration:none}.seg .on{background:#12345a;color:#58a8ff}.seg .count{background:#3a4250;color:#f1f4f8;padding:2px 7px}.seg .on .count{background:#873d45;color:#ffb1b7}.btn{border:1px solid #4776a8;background:#213a58;color:#dbeafe;border-radius:12px;padding:8px 13px;font-weight:800;text-decoration:none;white-space:nowrap;cursor:pointer}.btn.off{border-color:#3a4659;background:#1e2634;color:#d9e1ed}.btn.fail{border-color:#8e3b4b;color:#ff9faa}.table{width:100%;border-collapse:separate;border-spacing:0 8px}.head th{background:#1e2634;border-top:1px solid #293346;border-bottom:1px solid #293346;padding:10px;text-align:left;color:#aeb6c3;font-size:12px;text-transform:uppercase}.head th:first-child{border-left:1px solid #293346;border-radius:9px 0 0 9px}.head th:last-child{border-right:1px solid #293346;border-radius:0 9px 9px 0}.row td{background:#151b2a;border-top:1px solid #293346;border-bottom:1px solid #293346;padding:17px 10px;vertical-align:middle}.row td:first-child{border-left:1px solid #293346;border-radius:9px 0 0 9px}.row td:last-child{border-right:1px solid #293346;border-radius:0 9px 9px 0}.source{font-size:16px;font-weight:900}.sub{color:#9da7b8;font-size:12px;margin-top:6px}.model{font-family:ui-monospace,SFMono-Regular,Consolas,monospace;font-size:15px;font-weight:900}.effort{color:#48a6ff;font-weight:900}.recent{display:flex;gap:5px}.pulse{width:5px;height:22px;border-radius:999px;background:#5aca38}.pulse.bad{background:#fb7185}.ok{color:#5ee24b;font-weight:900}.badtext{color:#ff7c8f;font-weight:900}.num{font-size:16px}.usage{font-size:16px}.small{color:#9da7b8;font-size:12px}.metric{color:#61dc51;font-weight:900}.empty{padding:40px;text-align:center;color:#aeb7c6;border:1px dashed #344054;border-radius:9px;background:#151b2a}.details td{padding:0 10px 12px}.details pre{margin:0;max-height:260px;overflow:auto;white-space:pre-wrap;word-break:break-word;background:#0c1220;border:1px solid #2a3447;border-radius:7px;padding:12px;font-size:12px;line-height:1.45}.toggle{color:#91c9ff;cursor:pointer;text-decoration:underline;text-underline-offset:3px}@media(max-width:1100px){body{padding:14px}.bar{align-items:flex-start;flex-direction:column}.table{display:block;overflow:auto}.head th,.row td{white-space:nowrap}}</style>")
	body.WriteString("</head><body><main class=\"page\"><div class=\"bar\"><div class=\"title\"><h1>Venice Realtime</h1><span id=\"row-count-title\" class=\"count\">0</span></div><div class=\"actions\"><div class=\"seg\"><a href=\"/v0/resource/plugins/cpa-plugin-venice/accounts\">Accounts <span id=\"account-count\" class=\"count\">0</span></a><span class=\"on\">Realtime <span id=\"row-count\" class=\"count\">0</span></span></div><button id=\"mask\" class=\"btn\" type=\"button\">Masked</button><button id=\"failed\" class=\"btn fail off\" type=\"button\">Failed</button><button id=\"refresh\" class=\"btn\" type=\"button\">Refresh</button></div></div><div id=\"content\" class=\"empty\">Waiting for Venice requests...</div></main>")
	body.WriteString("<script>")
	body.WriteString(`(function(){let masked=true,failed=false,openRow="";const content=document.getElementById("content"),rowsBadge=document.getElementById("row-count"),titleBadge=document.getElementById("row-count-title"),acctBadge=document.getElementById("account-count"),maskBtn=document.getElementById("mask"),failedBtn=document.getElementById("failed"),refreshBtn=document.getElementById("refresh");function esc(v){return String(v==null?"":v).replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","\"":"&quot;","'":"&#39;"}[c]))}function fmtMS(v){v=Number(v||0);return v>=1000?(v/1000).toFixed(v>=10000?1:2)+" s":Math.round(v)+" ms"}function pct(v){return Number(v||0).toFixed(1)+"%"}function usage(u){u=u||{};return {i:Number(u.input_tokens||0),o:Number(u.output_tokens||0),t:Number(u.total_tokens||0)}}function time(v){if(!v)return "";const d=new Date(v);return d.toLocaleDateString()+"<br>"+d.toLocaleTimeString()}function bars(list){return '<div class="recent">'+(list||[]).map(ok=>'<span class="pulse '+(ok?'':'bad')+'"></span>').join("")+'</div>'}function detail(row){return '<tr class="details" id="d-'+esc(row.id)+'"><td colspan="11"><pre>'+esc(JSON.stringify(row.details||[],null,2))+'</pre></td></tr>'}function rowHTML(row,i){const u=usage(row.usage),id="r"+i+"-"+btoa(unescape(encodeURIComponent(row.source+row.model+row.time))).replace(/=+$/,"");row.id=id;const status=row.status==="Success"?'<span class="ok">Success</span>':'<span class="badtext">Failed</span>';return '<tr class="row"><td><div class="source">'+esc(row.source)+'</div><div class="sub">Provider: venice</div><div class="sub"><span class="toggle" data-id="'+esc(id)+'">Details</span></div></td><td><span class="model">'+esc(row.model)+'</span></td><td><div class="effort">'+esc(row.effort||"medium")+'</div><div class="small">Tier: '+esc(row.tier||"default")+'</div></td><td>'+bars(row.recent)+'</td><td>'+status+'</td><td><span class="'+(row.failed?'badtext':'ok')+'">'+pct(row.success_rate)+'</span></td><td class="num">'+esc(row.calls)+'</td><td class="num">'+Number(row.tps||0).toFixed(0)+'</td><td><span class="metric">'+fmtMS(row.ttft_ms)+'</span><span class="small"> | </span><span class="metric">'+fmtMS(row.elapsed_ms)+'</span></td><td>'+time(row.time)+'</td><td><div class="usage">'+(u.t>=1000?(u.t/1000).toFixed(1)+"K":u.t)+'</div><div class="small">I '+u.i+' / O '+u.o+'</div></td></tr>'+(openRow===id?detail(row):"")}function render(data){const rowCount=(data.summary&&data.summary.rows)||0;rowsBadge.textContent=rowCount;titleBadge.textContent=rowCount;acctBadge.textContent=(data.summary&&data.summary.accounts)||0;if(!data.rows||!data.rows.length){content.className="empty";content.textContent="Waiting for Venice requests...";return}content.className="";content.innerHTML='<table class="table"><thead class="head"><tr><th>Source</th><th>Model</th><th>Effort</th><th>Recent</th><th>Status</th><th>Success</th><th>Calls</th><th>TPS</th><th>TTFT | Elapsed</th><th>Time</th><th>Usage</th></tr></thead><tbody>'+data.rows.map(rowHTML).join("")+'</tbody></table>';content.querySelectorAll(".toggle").forEach(el=>el.onclick=()=>{openRow=openRow===el.dataset.id?"":el.dataset.id;load()})}async function load(){const url="realtime.json?rows=500&masked="+(masked?1:0)+"&failed="+(failed?1:0)+"&_="+Date.now();try{const res=await fetch(url,{cache:"no-store"});render(await res.json())}catch(e){content.className="empty";content.textContent=String(e&&e.message?e.message:e)}}maskBtn.onclick=()=>{masked=!masked;maskBtn.classList.toggle("off",!masked);maskBtn.textContent=masked?"Masked":"Unmasked";load()};failedBtn.onclick=()=>{failed=!failed;failedBtn.classList.toggle("off",!failed);load()};refreshBtn.onclick=load;load();setInterval(load,1000);})();`)
